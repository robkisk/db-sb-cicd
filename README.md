...
```bash
conda create -n db_sb_cicd python=3.9
conda activate db_sb_cicd
```
...
```
This command will create your personal repository under `/Repos/<username>/db_sb_cicd`.
...
- To trigger the release pipeline, get the current version from the `db_sb_cicd/__init__.py` file and tag the current code version:
```
